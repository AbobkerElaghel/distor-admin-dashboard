...
import { getSingleNews, updateNews } from '../../firebase/Firestore/NewsCollection';
...
const EditNews = ({ params }: any) => {
    ...
    const onNewsSubmit = async (e: any) => {
        ...
        if (photo && photo.length) {
            photoURL = await uploadPhotoAndGetUrl(params?.id, "news", photo[0]);
        }
        ...
        try {
            ...
            setSnackBarValue({ message: "Updated News successfully", severity: "success" }, 2000);
            setTimeout(() => {
                setLocation('/news');
            }, 2100)
        } catch (error: any) {
            setSnackBarValue({ message: error.message || 'Error in Editing a new News', severity: "error" }, 3000);
            console.dir(error)
            setSubmitting(false);
    ...
    return (
        <Box sx={{
            ...
            <SnackBarComponent />
            <Typography marginTop={7} marginBottom={3} fontWeight={500} variant='h3' component={"h3"}>{t('editNewsPage.editNews')}</Typography>
            <form onSubmit={onNewsSubmit}>
                <FormControl fullWidth>
                    ...
                    <Paper elevation={4} sx={{ p: 4, marginTop: 2 }}>
                        <Typography variant='h5' fontWeight={500} component={'h5'}>{t('editNewsPage.image')}</Typography>
                        <Typography variant='subtitle1' sx={{ opacity: 0.75 }}
                            fontWeight={500} component={'p'}>{t('editNewsPage.imageDescription')}</Typography>
                        {photoURL ? <ImageDropZone preview={photoURL} dispacther={setPhoto} /> : undefined}
                    </Paper>
                    <Paper elevation={4} sx={{ p: 4, marginY: 2 }}>
                        <Typography marginBottom={2} variant='h5' fontWeight={500} component={'h5'}>{t('editNewsPage.date')}</Typography>
                        <FormGroup sx={{
                            mb: 1
                        }}>
                            <FormControlLabel control={<Switch onChange={(e) => setIsAutoDate(e.target.checked)} name='autoDate' value={isAutoDate} />} label={t('editNewsPage.autoSelect')} />
                        </FormGroup>
                    ...
                    <Button disabled={submitting} type='submit' sx={{
                        mb: 3
                    }} variant='contained'>{t('editNewsPage.submit')}</Button>
                </FormControl>
            </form>
        </Box>
    )
