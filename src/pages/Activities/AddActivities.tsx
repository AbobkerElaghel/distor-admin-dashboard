...
const AddActivities = () => {
    ...
    const onActivitiesSubmit = async (e: any) => {
        ...
        if (!RichContent) {
            setSnackBarValue({ message: t('addActivitiesPage.feedbackActivityRichContentRequired'), severity: "error" }, 5000);
            return;
        };
        ...
        if (!photo.length) {
            setSnackBarValue({ message: t('addActivitiesPage.feedbackActivityPhotoRequired'), severity: "error" }, 5000);
            return;
        }
        if (!Auth?.user?.uid) {
            setSnackBarValue({ message: t('addActivitiesPage.feedbackActivityAuthRequired'), severity: "error" }, 5000);
            return;
        }
        try {
            setSubmitting(true);
            ...
            setSnackBarValue({ message: error.message || t('addActivitiesPage.feedbackActivityAddedError'), severity: "error" }, 3000);
            console.dir(error);
            setSubmitting(false);
    ...
    return (
        <Box sx={{
            ...
            <SnackBarComponent />
            <Typography marginTop={7} marginBottom={3} fontWeight={500} variant='h3' component={"h3"}>{t('addActivitiesPage.createNewActivity')}</Typography>
            <form onSubmit={onActivitiesSubmit}>
                <FormControl fullWidth>
                    ...
                    <Paper elevation={4} sx={{ p: 4, marginTop: 2 }}>
                        <Typography variant='h5' fontWeight={500} component={'h5'}>{t('addActivitiesPage.image')}</Typography>
                        <Typography variant='subtitle1' sx={{ opacity: 0.75 }}
                            fontWeight={500} component={'p'}>{t('addActivitiesPage.imageDescription')}</Typography>
                        <ImageDropZone dispacther={setPhoto} />
                    </Paper>
                    <Paper elevation={4} sx={{ p: 4, marginTop: 2 }}>
                        <Typography marginBottom={2} variant='h5' fontWeight={500} component={'h5'}>{t('addActivitiesPage.languages')}</Typography>
                        ...
                        <Autocomplete
                            ...
                            renderInput={(params) => <TextField error={errors?.language} margin='dense' {...params} label={t('addActivitiesPage.languages')} />}
                        />
                    </Paper>

                    <Paper elevation={4} sx={{ p: 4, marginY: 2 }}>
                        <Typography marginBottom={2} variant='h5' fontWeight={500} component={'h5'}>{t('addActivitiesPage.date')}</Typography>
                        <FormGroup sx={{
                            mb: 1
                        }}>
                            <FormControlLabel control={<Switch onChange={(e) => setIsAutoDate(e.target.checked)} name='autoDate' defaultChecked />} label={t('addActivitiesPage.autoSelect')} />
                        </FormGroup>
                    ...
                    <Button disabled={submitting} type='submit' sx={{
                        mb: 3
                    }} variant='contained'>{t('addActivitiesPage.submit')}</Button>
                </FormControl>
            </form>
        </Box>
    );
